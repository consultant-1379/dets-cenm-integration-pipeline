...
def createHLCBuildJob(project_name) {
        ...
        pipeline_hlc_name=pipeline_name + "_update-HLC-version"
    ...
    stages {
       stage ('Clean workspace') {
         ...
         stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                        url: "${GERRIT_CENTRAL}/${sg_gerrit_repo}"
            }
        }
     stage('Update Chart.yaml file and precommit Verification') {
            steps {
                sh "${bob} update_hlc_version --chart-name \${pipeline_name} --gerrit-proj \${sg_gerrit_repo}"
            }
            post {
                failure {
                    script {
        ...
        stage('Push Chart.yaml to master') {
         steps {
               sh "cd  ${pipeline_name} && git push origin HEAD:master" 
           }
            post {
                 failure {
                     script {
...
class BobCommand {
    ...
    String toString() {
       ...
       pipelineJob(pipeline_hlc_name) {
        description ('ENM Cloud Native Build pipeline ' + pipeline_hlc_name + ' - autogenerated using JobDSL - all manual changes will be overwritten!')
        parameters {
